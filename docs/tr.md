...
**База данных**
- Хранилище для промежуточных и финальных результатов
...
1. **Инициализация задачи:**
   ...
   - Task Manager сохраняет документ и параметры задачи в БД, присваивает ID задачи
...
2. **Планирование и распределение Map-задач:**
   ...
   - Planner читает документ из БД и разбивает его на n чанков
...
6. **Завершение задачи:**
   ...
   - Planner отмечает задачу как выполненную в БД
   - Task Manager уведомляет пользователя о готовности результата
...
3. Planner обнаруживает сбой, но видит в БД сохраненные промежуточные результаты
